"""
Legacy CLI interface for personal-index.

Provides commands to add interests, run crawls, search, and view results.
This module wraps the old CLI for backward compatibility.
"""
...
from personal_index.crawler_legacy import WebCrawler
...
def crawl_run(urls, depth, no_filter):
    ...
    crawler = WebCrawler(
        config=config.crawler,
        content_filter=content_filter if not no_filter else None,
        search_index=search_index,
    )

    click.echo(f"Crawling {len(urls)} URL(s)...")

    async def run():
        return await crawler.crawl(list(urls), max_depth=depth)

    pages = asyncio.run(run())
    click.echo(f"Crawled {len(pages)} pages")
    click.echo(f"Stats: {crawler.stats}")
...
def search(query, limit):
    ...
    for i, result in enumerate(results, 1):
        click.echo(f"  {i}. [{result.relevance_score:.3f}] {result.title}")
...
@index.command("list")
@click.option("--limit", "-l", type=int, default=20, help="Max pages to list.")
def index_list(limit):
    """List indexed pages."""
    idx = get_search_index()
    pages = idx.list_pages(limit=limit)

    if not pages:
        click.echo("No pages in the index.")
        return

    for page in pages:
        click.echo(f"  [{page.score:.3f}] {page.title or page.url}")
...
def index_clear():
    ...
    click.confirm("Are you sure you want to clear the entire index?", abort=True)
    idx = get_search_index()
    idx.clear()
...
@click.option("--url", "-u", multiple=True, required=True, help="Seed URL.")
...
def schedule_add(name, url, interval):
    ...
    click.echo(f"  Interval: {interval}h")
...
def schedule_list():
    ...
    for job in jobs:
        ...
        click.echo(f"    Interval: {job.interval_hours}h")
        click.echo(f"    Last run: {job.last_run or 'never'}")
        click.echo(f"    Runs: {job.run_count}")
        click.echo()
