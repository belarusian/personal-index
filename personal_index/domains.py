...
class DomainManager:
    ...
    ``_has_whitelist`` is always ``any(r.allowed for r in _rules.values())``
    -- recomputed on load and on every rule removal, and set on every allow.
    """
    ...
    def remove(self, domain: str) -> bool:
        """Removes the rule for ``domain`` and re-derives the
