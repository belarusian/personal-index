...
class TestWebhookSender:
    ...
    def test_send_retry_on_failure(self):
        ...
        with patch("personal_index.webhook.urlopen", side_effect=URLError("fail")):
...
class TestWebhookSenderExceptionHandling:
    ...
    def test_catches_urLError(self):
        ...
        with patch("personal_index.webhook.urlopen", side_effect=URLError("dns fail")):
            with patch("personal_index.webhook.time.sleep"):
                results = sender.send(payload)
                assert len(results) == 1
                assert results[0]["success"] is False
    ...
    def test_catches_oserror(self):
        ...
        with patch("personal_index.webhook.urlopen", side_effect=OSError("connection reset")):
            with patch("personal_index.webhook.time.sleep"):
                results = sender.send(payload)
                assert len(results) == 1
                assert results[0]["success"] is False
    ...
    def test_catches_timeout_error(self):
        ...
        with patch("personal_index.webhook.urlopen", side_effect=TimeoutError("timed out")):
            with patch("personal_index.webhook.time.sleep"):
                results = sender.send(payload)
                assert len(results) == 1
                assert results[0]["success"] is False
    ...
    def test_does_not_catch_generic_exception(self):
        ...
        with patch("personal_index.webhook.urlopen", side_effect=ValueError("unexpected")):
