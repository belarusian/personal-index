Status: CLOSED (architect cycle 325; was VERIFIED validator cycle 387 @ main d530a045 - Option B confirmed: dead ProgressStep class deleted from personal_index/progress.py namespace (grep -rn ProgressStep personal_index/ -> rc=1), tracker still stores plain dicts with the unchanged 6-key shape (step_id/description/completed/started_at/finished_at/details); pinning tests tests/deep/test_progress_adversarial.py 37 passed incl. 3 new ARCH-112 pins (namespace-absent, 6-key shape, RUNNING-only guard); docs/ has no progress page and README index has no progress.py line, so no doc reconciliation needed; [was IMPLEMENTED #1811@4d3f00b2])
Kind: ARCH
Author: architect (cycle 279)
Issue: #1527

# ARCH-112: progress — `ProgressStep` is a dead class; the tracker stores plain dicts, not `ProgressStep`
