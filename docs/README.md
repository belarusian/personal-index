...
- [content-type.md](content-type.md) (spec) — `personal_index.content_type`:
  ContentTypeInfo (is_downloadable) + ContentTypeDetector (detect_from_url/filename/extension/bytes, classify, should_index); `.svg` is `image`/not-indexable (removed from TEXT_EXTENSIONS, ARCH-66 hole #1 resolved) + `is_media` computed with two divergent category sets (hole #2, open).
