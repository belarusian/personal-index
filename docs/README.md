...
- [pagination.md](pagination.md) (spec) — `personal_index.pagination`:
  PageParams (clamped page/per_page, offset/limit) + PageResult (total_pages/has_next/has_prev/next_page/prev_page/start_index/end_index/to_dict, one-way) + Paginator (get_page/total_items/total_pages/iterate_pages); Paginator clamps constructor per_page to max(1, min(per_page, max_per_page)) so every entry point operates on the same effective size and per_page=0 is clamped to 1 with no path raising (ARCH-58, resolved).
...
- [stats.md](stats.md) (spec) — `personal_index.stats`:
  StatsCollector (get_index_stats) + IndexStats; read-only index aggregate over a SearchIndex, no persistence; distinct from analytics (event log) and content_analytics (content items); interest stats are derived from CrawledPage.matched_interests, not an InterestStore — the dead public interest_store field and the never-produced CrawlStats type are removed (confirmed contract, ARCH-86 Option A).
- [metrics.md](metrics.md) (spec) — `personal_index.metrics`:
  SystemMetrics (11-field @dataclass, to_dict) + MetricsCollector (increment_counter/set_gauge/record_histogram/collect_system_metrics/get_histogram_stats/get_report/reset); distinct from stats (index aggregate) and analytics (event log); collect_system_metrics populates uptime/memory_used/disk_* but cpu_percent and memory_total_mb are never collected and stay at their 0.0 defaults — memory_total_mb is kept (serialized by to_dict, part of the pinned key set) and documented as uncollected in the docstring (confirmed contract, ARCH-88 Option a).
- [performance-monitor.md](performance-monitor.md) (spec) — `personal_index.performance_monitor`:
  MetricSample + MetricStats (mean/stddev/p50/p95/p99 approximations) + PerformanceMonitor (record/timer/get_stats/get_all_stats/reset/get_recent_samples) + TimerContext; distinct from metrics (SystemMetrics/MetricsCollector) and stats (index aggregate); window_size bounds the _samples ring but NOT the _stats aggregate — get_stats() is a lifetime count/mean/min/max over all recorded values, not the retained window (confirmed contract, ARCH-89).
...
- [interests.md](interests.md) (spec) — `personal_index.interests`:
  InterestStore (add/remove/get/list_all/get_enabled/toggle, get_all_keywords/get_all_url_patterns/get_all_topics, update_priority/matches_any/clear/total_score, JSON persistence); distinct from content_scoring/content_filter/content_tagger/content_priority (which consume it); _load degrades to empty for corrupt/non-dict/null/non-dict-value/missing-name AND a valid-JSON-dict record with an out-of-enum interest_type or match_mode (ValueError now in the except tuple, ARCH-85 Option A) — the constructor never raises on file contents.
...
- [cli_verify.md](cli_verify.md) (spec) — `personal_index.cli_verify`:
  the `verify` click command + `_check_*`/`_run_*`/`_verify_*` self-test helpers (data-dir/interest/tag/search-index/filter/scorer checks + full-pipeline self-test); distinct from `personal_index.cli` (cli.md, the main group + subgroups) and the pipeline modules (which run the pipeline; this only checks it); RESOLVED — Option A (dead `_run_filter` deleted, cycle 261 / #1564@dcf0461), the full pipeline calls the `bool`-returning `_verify_filter` (line 291) (ARCH-97, VERIFIED).
- [cli_dedup.md](cli_dedup.md) (spec) — `personal_index.cli_dedup`:
  the `dedup` click command + `_load_indexed_content`/`_build_dedup_items`/`_dispatch_dedup`/`_display_result`/`_display_duplicate_groups`/`_remove_duplicates` helpers (hash/url/similarity/all dedup over indexed pages, dry-run guard); distinct from `personal_index.cli` (cli.md, the main group) and the underlying engine `personal_index.content_dedup` (dedup.md); the `Threshold:` line (line 118) prints `DuplicateGroup.similarity_score`, which for similarity groups is the configured `similarity_threshold` (content_dedup.py), not the measured Jaccard overlap (ARCH-102, resolved Option A: field docstring + `Score:`->`Threshold:` relabel, no behavior change).
...
- [domains.md](domains.md) (spec) — `personal_index.domains`:
  DomainRule (to_dict/from_dict, non-mapping degrades to empty-domain rule, unknown keys ignored) + DomainManager (add_allow/add_block/is_allowed/is_blocked/record_page/get_page_count/reset_counts/remove/list_rules/get_max_depth, JSON persistence, in-memory page counts); exact-match case-sensitive keys, whitelist = any allow rule, _has_whitelist always derived from the rule set (recomputed on load and after every removal, set on every allow) so removing the last allow rule restores the allow-all default (ARCH-84 Option A, confirmed contract).
...
- [url-filter.md](url-filter.md) (spec) — `personal_index.url_filter`:
  UrlFilterRule (pattern/description, matches: exact -> fnmatch -> re: regex via re.search, re.error swallowed to False) + UrlFilter (add_blacklist/add_whitelist, is_allowed/is_blocked/filter_urls/get_blocked_urls/get_matching_rule, blacklist_count/whitelist_count, clear/clear_blacklist/clear_whitelist); whitelist takes precedence over blacklist; block/allow is decided purely by list membership (the former is_blacklist field was removed — ARCH-87 Option A, confirmed contract).
...
- [versioning.md](versioning.md) (spec) — `personal_index.versioning` (DEAD module, 0 importers; distinct from the live `content_versioning` above): ContentVersion (7-field @dataclass keyed by url, to_dict) + VersionTracker (in-memory, url-keyed _versions; record_version/get_versions/get_latest/has_changed/get_change_count/get_all_urls/clear, total_versions/tracked_urls); in-memory only — no storage_path/_load/_save so nothing survives a restart, and no rollback_to/delete_version (only clear) — diverges from the live twin's JSON-backed item_id-keyed rollback/delete contract — ARCH-108 confirmed Option (b): the live `content_versioning` is the sole content-versioning contract and this dead module is marked for deletion in the IMPL lane (docs-only pass; no code change).
...
- [progress.md](progress.md) (spec) — `personal_index.progress` (DEAD module, 0 importers; distinct from the `on_progress` callback param on content_batch.BatchProcessor): ProgressState (6-state Enum) + ProgressStep (6-field @dataclass, to_dict) + ProgressTracker (state machine start/pause/resume/complete/fail/cancel, advance, progress_percent/elapsed_seconds/estimated_remaining, to_dict/from_dict lossy round-trip, format_bar) + ProgressStore (in-memory dict, optional JSON save_all/load_all, list_active/list_completed/cleanup); ProgressStep is a dead class — the tracker stores plain dicts (steps: list[dict], advance appends a dict), so the step model is split between a dead dataclass and a live ad-hoc dict (ARCH-109) contract hole.
...
- [export_markdown.md](export_markdown.md) (spec) — `personal_index.export_markdown`:
  ExportFormat (3-member str-Enum markdown/html/plain_text) + ExportConfig (5-field @dataclass, __post_init__ validates sort_by/group_by) + MarkdownExporter (export dispatch, _sort_items/_group_items/_render_md_item/_truncate) + module-level export_to_md/save_markdown/export_markdown; export never raises on a well-formed list and returns "" on empty input; the content flag (code name `include_summary`, to be renamed `truncate_content` per the confirmed ARCH-82 Option A contract) is a *replace* flag — the default (False) keeps the FULL content and True replaces it with a lossy 200-char word-boundary truncation, applied identically by all three renderers (confirmed contract, ARCH-82).
- [formatter.md](formatter.md) (spec) — `personal_index.formatter`:
  11 module-level pure str-returning display helpers (format_search_results, format_interest, format_crawl_stats, format_index_page, format_schedule_job, format_table, format_duration, format_file_size, format_timestamp, truncate, highlight) — no classes, no state, no I/O; none raise on documented inputs (missing dict keys default, falsy/empty inputs hit a guard, unparseable timestamps degrade to the raw string), format_table is total on ragged rows — the table spans the widest row (n_cols = max(len(headers), max(len(r) for r in rows))), a row SHORTER than the table is padded with empty cells and a row LONGER than the header widens the table with its excess cells rendered in extra columns (confirmed contract, ARCH-83 Option A).
...
- [content-api.md](content-api.md) (spec) — `personal_index.content_api`:
  ContentAPI (handle_request + _match_route chain: health/stats/content CRUD/search/export) + RequestLogger middleware; `_validate_content` IS wired into the `_create_content`/`_update_content` request path, so a POST/PUT with a >200-char title or non-list tags returns 400 (confirmed contract, ARCH-68, verified by validator cycle 222).
...
- [cycle-signals.md](cycle-signals.md) (spec) — `personal_index.cycle_signals`:
  codemap signal extractor (build_tree/format_tree/load_codemap, signal_no_tests/oversized/dead_code/duplicates/errors/coverage S1-S6, extract/format_for_auditor/main CLI); build_tree keeps a package's own module alongside its children (a node that is both a module and a parent carries both its own modules list and its children, so stats.modules always agrees with the visible listing — confirmed contract, ARCH-90 Option b).
...
- [logging_config.md](logging_config.md) (spec) — `personal_index.logging_config`:
  logging configuration helper (setup_logging/get_logger); setup_logging resolves `level` via `getattr(logging, level.upper(), None)` (line 18) and raises `ValueError` on an unrecognized level string (e.g. "VERBOSE", "TRACE", a typo) instead of silently coercing to INFO (ARCH-95, verified — fail-loud contract pinned by tests/deep/test_logging_config_adversarial.py).
...
- [cli_export.md](cli_export.md) (spec) — `personal_index.cli_export`:
  the standalone `export_cmd` click command (markdown/json/csv/html + --tag/--query/--limit filters) and its private _load_pages/_dispatch_format/_export_* helpers; distinct from cli.py which defines its OWN registered `export` (line 431, markdown/json/csv only, no filters); ARCH-98 confirmed Option A (wire it): export_cmd is to be registered on the main group and cli.py's thinner duplicate export removed (the code change is the IMPL lane), making the full four-format + three-filter surface reachable; currently export_cmd has 0 production importers (dead code) and the module's tests import only the private helpers, never export_cmd.
...
- [cli_top.md](cli_top.md) (spec) — `personal_index.cli_top`:
  the `top_pages` click command (named "top"; --limit/--format/--data-dir) and its private _to_json/_print_text helpers; RESOLVED — Option A (delete the dead parallel implementation) chosen and executed: personal_index/cli_top.py + tests/test_cli_top.py deleted (validator cycle 267 @ ef8b363, impl328 #1572), so the LIVE inline `top` at cli.py:906 is the sole implementation and emits p.to_dict() only (no rank/total/tags); the divergent 6-key JSON contract is gone (ARCH-100, VERIFIED).
...
- [notifications.md](notifications.md) (spec) — `personal_index.notifications`:
  NotificationLevel/NotificationType (Enums) + Notification (to_dict/from_dict, `read: bool` flag, NO delivery-timestamp field) + NotificationHandler ABC (Console/File/InMemory) + NotificationManager (add_handler/remove_handler/add_filter/notify->int, notify_crawl_complete/error/new_content/interest_match); NEVER imported or registered (grep for `import notifications`/`from personal_index.notifications` across the package returns nothing), so it is a DEAD parallel implementation of the LIVE record-and-track `content_notifications.py` (rules + `delivered` flag, no dispatch) — the two share the class names `Notification`/`NotificationManager` but have divergent, incompatible contracts; the dead module's `read` flag is a bare boolean with no delivery timestamp, unlike the live twin's `delivered` + `delivered_at` (mark_all_read never stamps a time, ARCH-106) contract hole.
